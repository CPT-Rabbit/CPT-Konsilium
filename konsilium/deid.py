...
_FIELD_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    (
        "DOB",
        re.compile(
            r"\b(?:Geburtsdatum|Geboren(?:\s+am)?|geb\s*\.?)\s*:?[ \t]*"
...
def residue_report(text: str, policy: dict[str, str] | None = None) -> list[ResidueHit]:
    ...
    for line_number, raw_line in enumerate(text.splitlines(keepends=True), 1):
        ...
        for name, pattern in _RESIDUE_PATTERNS.items():
            ...
            if action != "ignore" and matches and not _institutional_residue(name, text, offset, matches):
...
def _replace_match(match: re.Match[str], kind: str, token, today: date) -> str:
    ...
    if kind in {"ADDR", "PHONE"} and _is_institutional_address(match.string, match.start(1)):
...
def _replace_entity(text: str, kind: str, value: str, token) -> str:
    pattern = re.compile(rf"(?<!\w){re.escape(value)}(?!\w)")
    parts = []
    start = 0
    for protected in _TOKEN_SPAN.finditer(text):
        parts.append(_replace_entity_segment(text[start:protected.start()], pattern, kind, value, token, text, start))
        parts.append(protected.group(0))
        start = protected.end()
    parts.append(_replace_entity_segment(text[start:], pattern, kind, value, token, text, start))
    return "".join(parts)


def _replace_entity_segment(segment: str, pattern: re.Pattern[str], kind: str, value: str, token, text: str, offset: int) -> str:
    def replace(match: re.Match[str]) -> str:
        position = offset + match.start()
        if kind in {"ADDR", "PHONE"} and _is_institutional_address(text, position):
            return match.group(0)
...
def _institutional_residue(name: str, text: str, offset: int, matches: list[re.Match[str]]) -> bool:
    ...
    if name == "DIGIT_RUN":
        return all(
            ...
            and _CONTACT_MARKER.search(_line_at(text, offset + match.start()))
